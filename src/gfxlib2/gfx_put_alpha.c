...
void fb_hPutAlpha(unsigned char *src, unsigned char *dest, int w, int h, int src_pitch, int dest_pitch, int alpha, BLENDER *blender, void *param)
{
	...
	static PUTTER *simd_putters[] = {
		fb_hPutPSetC, fb_hPutPSetC, NULL, fb_hPutAlpha4SIMD,
	};
#endif
#ifdef HOST_X86
	...
	if (!context->putter[PUT_MODE_ALPHA]) {
		...
		if (fb_hSimdAvailable())
			context->putter[PUT_MODE_ALPHA] = simd_putters;
		else
#endif
#ifdef HOST_X86
		if (__fb_gfx->flags & X86_MMX_ENABLED)
			context->putter[PUT_MODE_ALPHA] = mmx_putters;
		else
#endif
			context->putter[PUT_MODE_ALPHA] = c_putters;
	}
