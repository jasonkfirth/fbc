...
void fb_hPutAnd(unsigned char *src, unsigned char *dest, int w, int h, int src_pitch, int dest_pitch, int alpha, BLENDER *blender, void *param)
{
	...
	static PUTTER *simd_putters[] = {
		fb_hPutAndSIMD, fb_hPutAndSIMD, NULL, fb_hPutAndSIMD,
	};
#endif
#ifdef HOST_X86
	...
	if (!context->putter[PUT_MODE_AND]) {
		...
		if (fb_hSimdAvailable())
			context->putter[PUT_MODE_AND] = simd_putters;
		else
#endif
#ifdef HOST_X86
		if (__fb_gfx->flags & X86_MMX_ENABLED)
			context->putter[PUT_MODE_AND] = mmx_putters;
		else
#endif
			context->putter[PUT_MODE_AND] = c_putters;
	}
