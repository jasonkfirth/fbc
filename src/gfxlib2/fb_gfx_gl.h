...
extern char __fb_gl_extensions[FBGL_EXTENSIONS_STRING_SIZE];
extern void fb_hGL_NormalizeParameters(int gl_options);
extern void *fb_hGL_GetProcAddress(const char *proc);

/*
	OpenGL ES contexts do not provide the desktop compatibility entry points
	used by gfxlib2's optional OpenGL 2D bridge.  Context-only backends still
	share the public parameter and ScreenGLProc machinery, but do not compile
	the fixed-function texture presenter.
*/
