...
static void fb_sfxPlayRunForeground(int frames)
{
    ...
    tick_frames = (__fb_sfx && __fb_sfx->samplerate > 0)
        ? (__fb_sfx->samplerate / 200)
        : 220;
...
static void fb_sfxPlayStringImpl(int channel, const char *str, int default_foreground)
{
    ...
    st.octave = (__fb_sfx)
        ...
        : 4;
    st.default_length = 4;
    st.channel = (channel >= 0 && channel < FB_SFX_MAX_CHANNELS) ? channel : 0;
...
{
    fb_sfxPlayStringImpl(0, music, 1);
}
