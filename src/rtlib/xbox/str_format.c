...
#ifdef HOST_XBOX
static unsigned long long fb_xbox_pow10_ull( int exponent )
{
...
static int fb_xbox_uint_to_str( unsigned long long value, char *buffer )
{
	char temp[32];
	int len = 0;
	int i;
...
static int fb_xbox_frac_to_fixed_str( double value, char *buffer, int precision )
{
	...
	scale = fb_xbox_pow10_ull( precision );
...
static double fb_xbox_truncate_unreliable_digits( double value, int digits )
{
	double scale;
	...
	scale = (double)fb_xbox_pow10_ull( digits );
...
#ifdef HOST_XBOX
	len_frac = fb_xbox_frac_to_fixed_str( dblFrac, pachFracPart, precision );
#else
...
#ifdef HOST_XBOX
		len_fix = fb_xbox_uint_to_str( ullFix, pachFixPart );
#else
...
#ifdef HOST_XBOX
			/*
			 * FORMAT masks can request more integral mantissa digits than a
			 * double can represent exactly.  After exponent scaling those
			 * low bits are binary representation noise; the generic x87 path
			 * effectively leaves them as zeroes for the existing tests.
			 */
			if( pInfo->has_exponent &&
			    (pInfo->num_digits_frac == 0) &&
			    (pInfo->num_digits_fix > 15) ) {
				value = fb_xbox_truncate_unreliable_digits( value,
				                                           pInfo->num_digits_fix - 15 );
			}
#endif
