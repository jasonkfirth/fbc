/*
    ...
    File: aros/dev_pipe_open.c

    Purpose:

        Provide bounded AROS OPEN PIPE input without POSIXC popen().

    Responsibilities:

        - recognize the read-only "ls <file>" command form
        - validate that the requested file exists
        - expose a one-line listing through normal file-device hooks
        - reject unsupported modes and commands without launching a shell

    This file intentionally does NOT contain:

        - a general-purpose AROS shell emulation
        - output or bidirectional pipe support
        - behavior for other operating systems

    POSIXC's popen path can wait indefinitely for an AROS shell child.  The
    portable fbctests contract only requires listing one existing file, so a
    temporary stream provides the same observable input without a child task.
*/
...
#define FB_AROS_PIPE_TEMP_FILE "T:fbpipe.tmp"

/* ------------------------------------------------------------------------- */
/* File-device hooks                                                         */
/* ------------------------------------------------------------------------- */

static FB_FILE_HOOKS hooks_dev_aros_pipe = {
	fb_DevFileEof,
	fb_DevFileClose,
	fb_DevFileSeek,
	fb_DevFileTell,
					  ...
					  size_t destination_length )
{
	...
	if( (tolower( (unsigned char)command[0] ) != 'l') ||
	    ...
	    ((command[2] != '\0') &&
	     !isspace( (unsigned char)command[2] )) )
		return FALSE;
...
static int hOpenResult( FB_FILE *handle )
{
	...
	stream = fb_hOpenFile( FB_AROS_PIPE_TEMP_FILE, "rb" );
	if( stream == NULL )
	...
	stream = fb_hReopenFile( FB_AROS_PIPE_TEMP_FILE, "rt", stream );
	if( stream == NULL )
		return fb_ErrorSetNum( FB_RTERROR_FILENOTFOUND );
	...
	handle->hooks = &hooks_dev_aros_pipe;
					...
					size_t filename_length )
{
	...
	stream = fb_hOpenFile( FB_AROS_PIPE_TEMP_FILE, "wb" );
	if( stream == NULL )
	{
		FB_UNLOCK();
