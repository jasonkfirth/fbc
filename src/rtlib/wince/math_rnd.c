/*
 * FreeBASIC Windows CE random-number runtime
 * -------------------------------------------
 *
 * File: wince/math_rnd.c
 *
 * Purpose:
 *
 *     Implement RND and RANDOMIZE on the Windows CE runtime surface.
 *
 * Responsibilities:
 *
 *     - provide the CRT, fast, Mersenne Twister, QB, and real generators
 *     - seed generators from the available timer and process state
 *     - request real entropy through COREDLL's Unicode CryptoAPI entry point
 *
 * This file intentionally does NOT contain:
 *
 *     - Linux or Android entropy-provider policy
 *     - compiler-side RND lowering
 *     - emulator-specific random-number behavior
 */
...
static int hRefillRealRndNumber( void )
{
	...
	if( CryptAcquireContextW( &provider, NULL, 0, PROV_RSA_FULL, 0 ) == TRUE ) {
