...
static inline void fb_CondInit( void )
{
	...
	kernel32 = GetModuleHandleW( L"COREDLL.DLL" );
	procedure = NULL;
	if( kernel32 != NULL )
		procedure = GetProcAddressW( kernel32, L"SignalObjectAndWait" );
	if( (procedure != NULL) &&
...
static FBCALL void fb_CondCreate_nt( FBCOND *cond )
{
	...
	/* Use COREDLL's native wide-character entry points.  Some MIPS images do
	   not provide the optional ANSI-name compatibility aliases. */
	cond->nt.sema = CreateSemaphoreW( NULL, 0, 0x7fffffff, NULL );
	cond->nt.waiters_done = CreateEventW( NULL, FALSE, FALSE, NULL );
}
...
static FBCALL void fb_CondCreate_9x( FBCOND *cond )
{
	cond->w9x.event[SIGNAL]    = CreateEventW( NULL, FALSE, FALSE, NULL );
	cond->w9x.event[BROADCAST] = CreateEventW( NULL, TRUE, FALSE, NULL );
}
