/*
    ...
    Purpose:
        ...
        Format floating-point values with significant-digit semantics on a
        C runtime whose printf implementation does not support %g correctly.

    Responsibilities:

        * round a value to the requested number of significant digits
        * select decimal or exponent notation using the C %g thresholds
        * remove insignificant trailing zeroes
        * format NaN, infinity, signs, and the optional QB leading blank

    This file intentionally does NOT contain:

        * FreeBASIC string allocation
        * locale-dependent decimal separators
        * Coredll or CeGCC printf delegation
*/
...
static char *hFloat2StrWince( double val,
                              char *buffer,
                              int digits,
                              int mask )
{
...
{
	return hFloat2StrWince( val, buffer, digits, mask );
}
