/*
 * Project: FreeBASIC RISC OS formatted numeric output
 * ---------------------------------------------------
 *
 * File: io_printusg.c
 *
 * Purpose:
 *
 *     Implement PRINT USING with the APCS double representation.
 *
 * Responsibilities:
 *
 *     - format numeric and string values for PRINT USING
 *     - interpret RISC OS APCS doubles as canonical IEEE-754 bit patterns
 *     - preserve the established FreeBASIC formatting rules
 *
...
{
	return fb_riscos_DoubleToBits( d );
}
