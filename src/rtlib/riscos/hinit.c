/*
 * Project: FreeBASIC RISC OS UnixLib runtime initialization
 * ---------------------------------------------------------
 *
 * File: hinit.c
 *
 * Purpose:
 *
 *     Own console initialization and runtime locks for UnixLib.
 *
 * Responsibilities:
 *
 *     - initialize runtime locks and the background console thread
 *     - configure the UnixLib terminal without unsupported process APIs
 *     - use the fixed TaskWindow geometry when size queries are unsafe
 *     - provide the process-owned termcap compatibility variables
 *
 * This file intentionally does NOT contain:
 *
 *     - Wimp application lifecycle
 *     - native graphics or pointer events
 *     - other Unix platform compatibility
 */
...
/*
 * UnixLib termcap compatibility
 *
 * UnixLib implements the termcap functions and declares the historical
 * writable PC, BC, and UP variables, but it does not define storage for them.
 * The termcap ABI requires process-global definitions, so this is the single
 * runtime-owned definition site for RISC OS builds.
 */
char PC;
char *BC;
char *UP;
 ...
 *
 * The UnixLib console backend restores terminal state before re-raising fatal
...
/**
 * Leave the tracked cursor position unchanged.
 *
 * UnixLib's TaskWindow terminal does not provide the cursor query protocol
 * used by the Linux console backend. The runtime instead maintains the cursor
 * position as it writes output.
 */
 ...
 *    riscos/io_mouse.c:mouse_handler() from the background thread
...
void fb_hRecheckConsoleSize( int requery_cursorpos )
{
	...
	/* Use the fixed TaskWindow terminal size. */
	struct winsize win = { 0, 0, 0, 0 };
	/*
	 * UnixLib's TaskWindow terminal does not provide a dependable
	 * TIOCGWINSZ query. Use the same geometry as the existing failure
	 * fallback without issuing an ioctl which may not return.
	 */
	win.ws_row = 25;
	win.ws_col = 80;
...
int fb_hInitConsole(void)
{
	...
	/* UnixLib provides getpgrp() but does not export getpgid(). */
...
static void hInit( void )
{
	...
	/* Trigger console window size & cursor position checks the first time
	   ...
	   output could be misinterpreted. In that case, at least, we'll only emit
	   them when the FB program uses console I/O commands, but not on start up of
	   every FB program. */
	__fb_console_resized = TRUE;
