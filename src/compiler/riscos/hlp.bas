...
function hFloatToHex _
	...
	''
	'' RISC OS APCS stores the most-significant double word first.  Reading
	'' it as a little-endian ULongInt therefore requires a word swap before
	'' the compiler serializes the IEEE-754 bit pattern.
	if( typeGet( dtype ) = FB_DATATYPE_DOUBLE ) then
		dim as ulongint bits = *cptr( ulongint ptr, @value )
		bits = (bits shl 32) or (bits shr 32)
		function = "0x" + hex( bits, 16 )
	else
...
function hFloatToHex_C99 _
	...
	'' See hFloatToHex(): APCS double words must be normalized before
	'' extracting sign, exponent, and mantissa for a C99 hexadecimal literal.
	dim as ulongint n = *cptr( ulongint ptr, @value )
	n = (n shl 32) or (n shr 32)
