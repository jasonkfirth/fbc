...
'' Resource ownership:
''
'' p1 owns manually constructed objects and is released with their matching
'' destructors followed by Deallocate.  p2 is a New[] allocation owned by
'' Delete[].  The objects placed in array must be destructed before its storage
'' is erased.  p4 owns its dynamic array storage until Erase.
...
If p1 = 0 Then
	Print "Could not allocate the first object array"
Else
	For I As Integer = 0 To 2
		p1[I].Constructor()                    '' call the constructor on each element
	Next I
	For I As Integer = 0 To 2
		'' This fixed three-element tutorial loop appends one short index per object.
	...
	Next I
	Print
	Dim As UDT Ptr p1_resized = Reallocate(p1, 4 * SizeOf(UDT))
	If p1_resized = 0 Then
		Print "Could not resize the first object array"
		For I As Integer = 0 To 2
			p1[I].Destructor()
		Next I
		Deallocate(p1)
		p1 = 0
	Else
		p1 = p1_resized                         '' retain ownership only after Reallocate succeeds
...
If p2 = 0 Then
	Print "Could not allocate the second object array"
Else
	For I As Integer = 0 To 2
		'' This fixed three-element tutorial loop appends one short index per object.
		'' FB-LINTER: DISABLE-NEXT-LINE FBL503
...
'' array is exactly three UDT-sized slots, and the destructor loop below runs before Erase.
'' FB-LINTER: DISABLE-NEXT-LINE FBL525 FBL-PTR-023
