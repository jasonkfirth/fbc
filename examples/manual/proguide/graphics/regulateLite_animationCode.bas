...
#define vct Type<vector3d>
 ...
 'macros
	...
	Function apply_perspective(ByRef p As vector3d, ByRef eyepoint As vector3d) As vector3d
		Const As Single minimumProjectionDepth = .00001
		If Abs(eyepoint.z) < minimumProjectionDepth Then Return p
		Dim As Single w=1+(p.z/eyepoint.z)
		If Abs(w) < minimumProjectionDepth Then
			If w < 0 Then
				w = -minimumProjectionDepth
			Else
				w = minimumProjectionDepth
			End If
		End If
	...
	' The helpers and renderer are in one source module and share only this frame state.
	' FB-LINTER: DISABLE-NEXT-LINE FBL301
	Dim Shared As Integer xres, yres
	If ScreenRes(800, 640, 8) <> 0 Then
	...
	'two main arrays shared by the helpers and the frame renderer in this module
	' FB-LINTER: DISABLE-NEXT-LINE FBL301
	...
	Function create1(ByVal number As Integer) As Integer
		Const As Integer stepper = 10
		Dim As Integer count
		Dim As Integer pointsPerSide
		Dim As Integer pointCount
		If number < 0 Then Return 0
		pointsPerSide = (number * 2) \ stepper + 1
		pointCount = pointsPerSide * pointsPerSide * pointsPerSide
		ReDim array(1 To pointCount)
		For x As Integer=xres \ 2-number To xres \ 2+number Step stepper
			For y As Integer=yres \ 2-number To yres \ 2+number Step stepper
				For z As Integer=-number To number Step stepper
					count += 1
					array(count)=vct(x, y, z)
				Next z
			Next y
		Next x
		ReDim rotated(1 To count)
		Return count
	End Function
	...
	Dim As Boolean remove=False
	Dim As Boolean skipped
	Dim As Long ist, mist
	Dim As ULong averageFps
	Dim As Double sumFps
	Dim As Long N
	Dim As vector3d disp=vct(xres/2, yres/2, 0)
	Dim As Integer border=xres \ 5
	Dim As Single sx=1
	Dim As Single kx=2
	Dim As Single ky=1.9
	Dim As Integer shapeCount=create1(90)
	If shapeCount = 0 Then
		Print "Could not create the animation point cloud"
		Sleep
		End
	End If
	Do
		...
		If (remove = False) Or (skipped = False) Then
			...
			For n As Integer=1 To shapeCount
				rotated(n)=rotate3d(centre, (array(n)), angle, vct(sx, sx, sx))
			Next n

			combsort(rotated, 1, shapeCount, .z)
