...
Sub printArrayDescriptor (ByVal p As Any Ptr, ByVal tabulation As Integer = 0, ByRef title As String = "")
	...
	Dim As Integer t = 0
	If pd = 0 Then
		Print "No array descriptor is available"
		Exit Sub
	End If
	If title <> "" Then
		Print title
	...
	'' FBARRAY stores these descriptor addresses as Any Ptr by runtime design.
	'' FB-LINTER: DISABLE-NEXT-LINE FBL-PTR-013
...
Sleep
Cls
'' The unsized declaration is the descriptor state demonstrated below.
