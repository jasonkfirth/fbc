...
Options:
  ...
  --release NAME    Limit tests to one release
...
run_fbctests() {
    ...
    run apk add --no-cache make g++ tar
...
run fbc /tmp/fb-package-smoke/gfx-screen13.bas -x /tmp/fb-package-smoke/gfx-screen13 -v
...
(
    ...
    run fbc showcase.bas -x /tmp/fb-package-smoke/sfx-showcase -v
)
[ -x /tmp/fb-package-smoke/sfx-showcase ] || fail "sfxlib showcase binary was not created"

echo "==> running sfxlib smoke"
if ! SFXLIB_DRIVER=null run_timeout /tmp/fb-package-smoke/sfx > /tmp/fb-package-smoke/sfx.out 2> /tmp/fb-package-smoke/sfx.err; then
