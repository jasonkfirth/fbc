...
# --------------------------------------------
#
# File: wince/run-mips-oma-games.sh
#
# Purpose:
#
#     Launch-qualify every available OMA game in the Windows CE MIPS emulator.
#
# Responsibilities:
#
...
CERF_ROOT="${WINCE_CERF_ROOT:-$ROOT/out/wince/emulator/cerf-mips-nec_rockhopper_mips2_ce6}"
MIPS_BOARD_ID="${WINCE_MIPS_BOARD_ID:-nec_rockhopper}"
MIPS_ROM="${WINCE_MIPS_ROM:-$CERF_ROOT/roms-wince-mips.bin}"
OMA_ROOT="${WINCE_OMA_OUTDIR:-$ROOT/out/wince/oma/mips}"
...
usage() {
	cat <<EOF
Usage: ./build_scripts/wince/run-mips-oma-games.sh [options]
...
Options:
  ...
  --out-dir DIR      Evidence root. Default: out/wince/oma/mips/tests
...
for tool in awk clang cp docker find grep mkdir mktemp rm sha256sum sort tr xargs; do
	require_command "$tool"
done
[ -x "$SCRIPT_DIR/build-oma-games.sh" ] ||
	die "Windows CE OMA build script is not executable"
[ -x "$SCRIPT_DIR/run-mips-emulator.sh" ] ||
	die "Windows CE MIPS emulator runner is not executable"
[ -x "$MIPS_COMPILER" ] || die "Windows CE MIPS compiler wrapper is missing"
[ -x "$MIPS_TOOLCHAIN_ROOT/bin/mips-wince-pe-objdump" ] ||
	die "Windows CE MIPS object inspector is missing"
...
if [ "$BUILD_GAMES" -eq 1 ]; then
	"$SCRIPT_DIR/build-oma-games.sh" --arch mips
fi
...
msg "building Windows CE MIPS OMA launch runner"
clang --target=mipsel-pc-windows-msvc \
	...
	-c "$ROOT/tests/wince/oma-runner.c" \
	-o "$OUTPUT_ROOT/oma-runner.o"
"$MIPS_COMPILER" -O 0 "$OUTPUT_ROOT/oma-runner.o" \
	-x "$OUTPUT_ROOT/oma-runner.exe"
"$MIPS_TOOLCHAIN_ROOT/bin/mips-wince-pe-objdump" \
	-f "$OUTPUT_ROOT/oma-runner.exe" |
	grep -F 'file format pei-mips' >/dev/null ||
	die "OMA launch runner is not a MIPS PE executable"
...
save_evidence() {
	...
	if [ -f "$CERF_ROOT/logs/oma-mips-$key.log" ]; then
		cp "$CERF_ROOT/logs/oma-mips-$key.log" "$OUTPUT_ROOT/logs/$key.log"
	fi
	if [ -f "$CERF_ROOT/logs/oma-mips-$key.png" ]; then
		cp "$CERF_ROOT/logs/oma-mips-$key.png" \
...
for row in "${RUN_ROWS[@]}"; do
	...
	msg "launching $display_name on Windows CE MIPS"
	stage_game "$key" "$game_directory"
	#
	# Duel generates its rotated sprite set at startup, and QFAK performs a
	# long scripted world transition.  CERF's emulated MIPS CPU and guest clock
	# can take substantially longer than native elapsed time for those phases,
	# although the same releases reach gameplay consistently afterward.
	# This is only a host completion allowance; the guest still has to satisfy
	# every process, window, input, gameplay, and stability requirement.
	#
	WINCE_CERF_ROOT="$CERF_ROOT" \
	WINCE_MIPS_BOARD_ID="$MIPS_BOARD_ID" \
	WINCE_MIPS_ROM="$MIPS_ROM" \
	"$SCRIPT_DIR/run-mips-emulator.sh" \
		--program oma-runner.exe \
		--completion oma.done \
		--log-stem "oma-mips-$key" \
		--boot-seconds "$BOOT_SECONDS" \
		--run-seconds "$((STABILITY_SECONDS + 1200))"
...
cat > "$REPORT" <<EOF
# Windows CE MIPS OMA gameplay execution
...
echo
echo "==> Windows CE MIPS OMA gameplay qualification passed: ${#RUN_ROWS[@]}/${#RUN_ROWS[@]} games"
echo "    Execution report: $REPORT"

# end of build_scripts/wince/run-mips-oma-games.sh
