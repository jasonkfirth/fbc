...
build_bootstrap_tarball() {
    ...
    rm -f "$BOOTSTRAP_TAR"
    fb_remove_build_tree "$ROOT" "$ROOT/bootstrap/${BOOTKEY}" || die "could not remove bootstrap/${BOOTKEY}"
...
screen 0
SMOKEEOF
cat > .package-smoke/gfx3-screen13.bas <<'SMOKEEOF'
screen 13
screenset 0, 0
pset (8, 8), 12
...
bin/fbc -v -prefix "\$PWD" -i inc -gfx3 .package-smoke/gfx3-screen13.bas -x .package-smoke/gfx3-screen13
