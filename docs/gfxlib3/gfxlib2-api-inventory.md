...
| `FB.PaintPattern` | `fb_GfxPaintPattern` | Both libraries accept 1..64 packed two-color rows; gfxlib3 uses its CPU flood and surface transfer path. See [the compatibility services guide](../compatibility-services.md). |
...
| `FB.DrawStringSize` | `fb_GfxDrawStringSize` | Both libraries measure native byte-font advances, sharing width-table validation with rendering. See [the compatibility services guide](../compatibility-services.md). |
