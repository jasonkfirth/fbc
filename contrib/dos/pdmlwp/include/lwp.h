...
/* --------------From now on, can be freely modified--------------*/
	...
	union {
      ...
      unsigned long wakeup_tick; /* RTC deadline; retains the union's ABI. */
   } waiting;
	...
	int saved_errno;
	unsigned short saved_dpmi_error;
} lwp;
...
/* Optional fast RTC consumer. The caller locks its code/data before installing
 * it and releases it before unlocking them. The callback runs with interrupts
 * disabled on a private locked stack: no FPU, allocation, DOS, or scheduling.
 * Only one consumer is supported. Scheduling retains the lwp_init() rate.
 * Return 1 on success, 0 if the RTC or consumer is unavailable.
 */
int lwp_set_tick_hook(void (*hook)(void), int speed);
int lwp_clear_tick_hook(void (*hook)(void));
