''
'' FreeBASIC Windows CE thread race tests
'' --------------------------------------
''
'' File: tests/wince/threads/racecondition.bas
''
'' Purpose:
''
''     Exercise compiler and runtime race-condition regressions with a
''     Windows CE-safe number of simultaneously reserved thread stacks.
''
'' Responsibilities:
''
''     - create and join multiple independent worker threads
''     - verify SELECT AS CONST temporary storage is thread-local
''     - preserve genuine concurrent execution within the CE process slot
''
'' This file intentionally does NOT contain:
''
''     - desktop-sized thread fan-out
''     - DOS target exclusions
''     - general mutex stress covered by threads/stress.bas
''
...
SUITE( fbc_tests.threads.racecondition )
	...
	TEST_GROUP( multiplethreads )
		'' Keep simultaneous stack reservations within the Windows CE process
		'' slot while retaining enough workers to expose shared-temp races.
		const NUM_THREADS = 8
...
'' end of tests/wince/threads/racecondition.bas
