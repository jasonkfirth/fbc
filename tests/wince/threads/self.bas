''
'' FreeBASIC Windows CE thread identity tests
'' ------------------------------------------
''
'' File: tests/wince/threads/self.bas
''
'' Purpose:
''
''     Verify ThreadSelf, ThreadWait, and ThreadDetach inside the Windows CE
''     process-slot and per-thread stack limits.
''
'' Responsibilities:
''
''     - compare ThreadCreate handles with ThreadSelf results
''     - exercise joined and detached thread ownership
''     - retain real concurrent fan-out without exhausting a CE process slot
''
'' This file intentionally does NOT contain:
''
''     - desktop-sized thread fan-out
''     - DOS or JavaScript target policy
''     - mutex contention stress already covered by threads/stress.bas
''
...
SUITE( fbc_tests.threads.self )
	...
	TEST_GROUP( idBasic )
		'' Windows CE 5 assigns every thread a reserved stack inside the process
		'' slot. Eight live threads retain concurrent coverage without exhausting
		'' that bounded virtual address space before the assertions can run.
		const NUM_THREADS = 8
...
'' end of tests/wince/threads/self.bas
