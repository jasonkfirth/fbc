''
'' FreeBASIC AROS fbctest replacement
'' ----------------------------------
''
'' File: threads/threadcall.bas
''
'' Purpose:
''
''     Exercise ThreadCall using the AROS ABI and target libffi builds.
''
'' Responsibilities:
''
''     - preserve the shared ThreadCall coverage
''     - exercise AROS libffi on x86_64, m68k, and ARM
''     - keep AROS target expectations out of the shared test
''
...
'' The shared test excludes ARM because most ARM targets do not provide a
'' tested libffi build. AROS stages target-specific libffi builds, so the
'' complete suite remains enabled for every supported AROS architecture.

#ifndef ENABLE_CHECK_BUGS
#define ENABLE_CHECK_BUGS 0
#endif

#if (not defined( __FB_DOS__ )) and (not defined( __FB_WII__ )) and ((not defined( __FB_JS__ )) or ( ENABLE_CHECK_BUGS <> 0 ))
