...
'' Link the instrumented gfx_paint object and paint-alloc.c; no device is used.
...
for allocation as long = 1 to 11
	line ( 0, 0 )-( 11, 9 ), 2, bf
	fail_at allocation
	if fb.PaintPattern( 0, 5, 5, chr( 255, 0, 128 ), 1, 0, 3 ) <> 4 then end 2
...
next
fail_at 0
if fb.PaintPattern( 0, 5, 5, chr( 255 ), 1, 0, 3 ) <> 0 then end 5
