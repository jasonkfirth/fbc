''
'' FreeBASIC RISC OS fbcunit core
'' ------------------------------
''
'' File: fbcunit.bas
''
'' Purpose:
''
''     Provide the fbcunit implementation for the APCS double layout.
''
'' Responsibilities:
''
''     - implement fbcunit suite and assertion behavior
''     - compare doubles using canonical IEEE-754 bit order
''     - preserve the shared fbcunit reporting interface
''
...
namespace fbcu
	...
	private function dblToBits _
		...
		dim words as ulong ptr = cast( ulong ptr, @a )
		return (culngint(words[0]) shl 32) or words[1]
	...
	private function dblFromBits _
		...
		dim value as double
		dim words as ulong ptr = cast( ulong ptr, @value )
		words[0] = culng(bits shr 32)
		words[1] = culng(bits)
		return value
